...
class EightSleepAPIClient:
    ...
    def get_sleep_session(self, device_id: str, date: str) -> dict:
        ...
        """
        # TODO: Implement actual API call
        return {"intervals": [], "score": None, "duration": None}
...
class EightPlugin(SleepTrackerPlugin):
    ...
    def __init__(self, console):
        """Initialize the Eight Sleep plugin with the correct name."""
    ...
    def get_api_client(self) -> EightSleepAPIClient:
        ...
        if not (
            ...
            raise APIError(
                "EIGHT_USERNAME and EIGHT_PASSWORD environment variables must be set"
            )
    ...
    def get_device_ids(
        ...
        if self.device_id:
            ...
            self.console.print(
                f"📱 Using configured Eight Sleep device ID: {self.device_id}"
            )
        ...
        if auto_discover:
            try:
                ...
                if device_ids:
                    self.console.print(
                        f"📱 Auto-discovered Eight Sleep devices: {', '.join(device_ids)}"
                    )
                    return device_ids, device_names
                else:
                    ...
                    self.console.print(
                        f"📱 Auto-discovered Eight Sleep device: {device_id}"
                    )
        ...
        with Progress(
            ...
            while current_date <= end_date:
                ...
                try:
                    ...
                    logging.error(
                        f"Error fetching Eight Sleep data for {current_date.date()}: {e}"
                    )
    ...
    def discover_devices(self) -> None:
        ...
        try:
            ...
            for device in devices:
                self.console.print(
                    f"Device: {device['name']} (ID: {device['device_id']}, Model: {device['model']})"
                )
