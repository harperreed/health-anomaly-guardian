...
from anomaly_detector.plugins.eight import EightPlugin, EightSleepAPIClient
...
class TestEightPlugin:
    ...
    def setup_method(self):
        ...
        with patch(
            "anomaly_detector.plugins.eight.get_env_var",
    ...
    def test_cache_key_generation(self):
        ...
        assert cache_key == f"eight_{device_id}_{date_str}"
    ...
    def test_get_api_client_with_credentials(self):
        ...
        assert isinstance(api_client, EightSleepAPIClient)
...
class TestEightSleepAPIClient:
    """Test the Eight Sleep API client functionality."""

    def setup_method(self):
        """
        Set up the API client instance for each test method.
        """
        self.api_client = EightSleepAPIClient("test_user", "test_pass")

    def test_initialization(self):
        """
        Verify that the API client is initialized with the correct username and password.
        """
        assert self.api_client.username == "test_user"
        assert self.api_client.password == "test_pass"

    def test_get_devices(self):
        """
        Tests that the API client returns a list containing the default Eight Sleep device with the expected device ID and name.
        """
    ...
    def test_get_sleep_session(self):
        """
        Tests that retrieving a sleep session with the API client returns placeholder data structure.
        """
        sleep_data = self.api_client.get_sleep_session("device_id", "2024-01-01")

        assert isinstance(sleep_data, dict)
        assert "intervals" in sleep_data
        assert "score" in sleep_data
        assert "duration" in sleep_data
        assert sleep_data["intervals"] == []
        assert sleep_data["score"] is None
        assert sleep_data["duration"] is None
...
class TestEightPluginExtended:
    ...
    def test_plugin_initialization_partial_env_vars(self):
        ...
        partial_env = {"EIGHT_USERNAME": "test_user", "EIGHT_PASSWORD": "test_pass"}
        with patch(
            "anomaly_detector.plugins.eight.get_env_var",
            side_effect=lambda key, default=None: partial_env.get(key, default),
        ):
            plugin = EightPlugin(self.console)
    ...
    def test_get_device_ids_with_whitespace_device_id(self):
        ...
        assert device_ids == ["  test_device_123  "]
        assert device_names == {
            "  test_device_123  ": "Eight Sleep Pod (  test_device_123  )"
        }
...
class TestEightSleepAPIClientExtended:
    """Extended comprehensive tests for the Eight Sleep API client."""
    ...
    def test_initialization_with_empty_credentials(self):
        """Test API client initialization with empty credentials."""
        api_client = EightSleepAPIClient("", "")

        assert api_client.username == ""
        assert api_client.password == ""

    def test_initialization_with_none_credentials(self):
        """Test API client initialization with None credentials."""
        api_client = EightSleepAPIClient(None, None)

        assert api_client.username is None
        assert api_client.password is None

    def test_initialization_with_special_characters(self):
        """Test API client initialization with special characters in credentials."""
        username = "test@user!123"
        password = "p@ssw0rd!#$%"
        api_client = EightSleepAPIClient(username, password)
    ...
    def test_get_devices_multiple_calls(self):
        ...
        devices1 = self.api_client.get_devices()
        devices2 = self.api_client.get_devices()
    ...
    def test_get_sleep_session_various_inputs(self):
        ...
        result1 = self.api_client.get_sleep_session("device_123", "2024-01-01")
        assert isinstance(result1, dict)
        assert "intervals" in result1

        # Test with empty inputs
        result2 = self.api_client.get_sleep_session("", "")
        assert isinstance(result2, dict)
        assert "intervals" in result2

        # Test with None inputs
        result3 = self.api_client.get_sleep_session(None, None)
        assert isinstance(result3, dict)
        assert "intervals" in result3

        # Test with special characters
        result4 = self.api_client.get_sleep_session("device!@#", "2024-12-31")
        assert isinstance(result4, dict)
        assert "intervals" in result4
    ...
    def test_get_sleep_session_date_formats(self):
        ...
        for date_str in date_formats:
            result = self.api_client.get_sleep_session(device_id, date_str)
            assert isinstance(result, dict)
            assert "intervals" in result

    def test_api_client_immutability(self):
        """Test that API client credentials can be accessed after initialization."""
        # Try to modify (should not affect the object if properly designed)
        try:
            self.api_client.username = "modified_user"
            self.api_client.password = "modified_pass"
        except AttributeError:
            # If the implementation prevents modification
            pass

        # Verify that the getter methods still work
        assert hasattr(self.api_client, "username")
        assert hasattr(self.api_client, "password")
...
class TestEightPluginErrorHandling:
    ...
    def test_fetch_data_with_corrupted_cache(self):
        ...
        # Should handle gracefully and raise appropriate error since no valid data is found
        with pytest.raises(DataError, match="No valid Eight Sleep data found"):
            self.plugin.fetch_data("test_device", start_date, end_date, cache)
...
class TestEightPluginIntegration:
    ...
    def test_cache_key_consistency_across_calls(self):
        ...
        assert key1 == key2
        assert key1 == f"eight_{device_id}_{date_str}"
    ...
    def test_environment_variable_dependency(self):
        ...
        for config in test_configs:
            with patch(
                "anomaly_detector.plugins.eight.get_env_var",
                side_effect=lambda key, default=None, cfg=config: cfg.get(key, default),
            ):
                plugin = EightPlugin(self.console)
